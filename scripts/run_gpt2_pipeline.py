...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a repeatable GPT-2 fine-tuning pipeline (dev-only).")
...
def build_effective_config(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    cfg: Dict[str, Any] = {
        "run_name": str(raw.get("run_name") or "gpt2-zh-placeholder-smoke"),
...
def main() -> None:
    ...
    corpus_dir = layout.edge_deid_datasets_home / "processed" / run_slug / split
    corpus_path = corpus_dir / "corpus.jsonl"
    if bool(cfg["force_prepare"]) or not corpus_path.exists():
        prepare_cmd = [
            sys.executable,
